...
---------------------");
	...
	let number_of_scale = convert_to_number(&scale[..1]);
	let note_char = &scale.to_uppercase()[1..2];
	let char_acc: char = scale[2..=2].parse().unwrap();

	if &scale.len() == &3_usize || &scale.len() == &4_usize {
		scale_name(number_of_scale, note_char, char_acc);
	} else if &scale.len() < &3_usize { println!("Less than 2 symbols. Please pick scale number and root note from the list") }
	else if &scale.len() > &4_usize { println!("More than 3 symbols. Please pick scale number and root note from the list") };

	constructor(number_of_scale, char_acc, &scale[1..]);
}

fn convert_to_number(scale: &str) -> usize {
    if scale == "1" { 1 } else if scale == "2" { 2 } 
    else if scale == "3" { 3 } else if scale == "4" { 4 } 
    else if scale == "5" { 5 } else if scale == "6" { 6 }
	else if scale == "7" { 7 } else { 0 }
}

fn scale_name(scale: usize, note: &str, char_acc: char) {
	let char_cleaned: char;
	if char_acc == '\n' { char_cleaned = ' ' } else { char_cleaned = char_acc }

	let mut scale = scale;
	let mut note_name = "x";

	if *note == *"C" { note_name = "C" } else if *note == *"D" { note_name = "D" } 
    else if *note == *"E" { note_name = "E" } else if *note == *"F" { note_name = "F" } 
    else if *note == *"G" { note_name = "G" } else if *note == *"A" { note_name = "A" }
	else if *note == *"H" { note_name = "H" } else { scale = 0 };
	match scale {
...
fn sharp_flats(temp: &std::vec::Vec<&str>) -> String {
	...
	let final_string = format!("{}, {}, {}, {}, {}, {}, {}", new_vec[0], new_vec[1], new_vec[2], new_vec[3], new_vec[4], new_vec[5], new_vec[6]);
	
	final_string
}

fn constructor<'a>(number_of_scale: usize, char_acc: char, scale: &str) -> String {
	let current_scale;
	let mut note_acc = scale[0..1].to_uppercase().to_owned();
	if char_acc == 'b' || char_acc == '#' {note_acc.push(char_acc)}
	
	println!("note_acc: {}", note_acc);

	note_acc = swapper( &note_acc ).to_string();

	if char_acc == 'b' { current_scale = ["C", "Db", "D", "Eb", "E", "F", "Gb", "G", "Ab", "A", "Hb", "H"] }
	else { current_scale = ["C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A", "A#", "H"] }
	...
	if temp.contains(&"C") && temp.contains(&"C#") {result = sharp_flats(&temp)}
	else if temp.contains(&"D") && temp.contains(&"D#") {result = sharp_flats(&temp)}
	else if temp.contains(&"E") && temp.contains(&"E#") {result = sharp_flats(&temp)}
	else if temp.contains(&"F") && temp.contains(&"F#") {result = sharp_flats(&temp)}
	else if temp.contains(&"G") && temp.contains(&"G#") {result = sharp_flats(&temp)}
	else if temp.contains(&"A") && temp.contains(&"A#") {result = sharp_flats(&temp)}
	else if temp.contains(&"H") && temp.contains(&"H#") {result = sharp_flats(&temp)}
	else { result = format!("{}, {}, {}, {}, {}, {}, {}", temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6]); }
